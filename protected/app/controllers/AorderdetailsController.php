<?php
class AorderdetailsController extends BaseController {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'aorderdetails';
	static $per_page	= '10';
	
	public function __construct() {
		parent::__construct();
		$this->model = new Aorderdetails();
		...
		$this->data = array(
			...
			'pageModule'		=> 'aorderdetails',
			'pageUrl'			=> URL::to('aorderdetails')		
		);
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','aorderdetails.index',$this->data);
	}		
	...
	function getAdd( $id = null)
	{
		...
		$this->data['id'] = $id;
		return View::make('aorderdetails.form',$this->data);
	...
	function postQuicksave( $id =0)
	{
		...
		$validator = Validator::make(Input::all(), $rules);	
		if ($validator->passes()) {	
			$data = $this->validatePost('OrderDetailsID');
	...
	function postCopy()
	{
        ...
        {
			if( $column->Field != 'OrderDetailsID')
		...
		$sql .= " SELECT ".implode(",", $columns)." FROM orderdetails WHERE OrderDetailsID IN (".$toCopy.")";
	//	DB::insert($sql);
		return Response::json(array(
			'status'=>'success',
			'message'=> SiteHelpers::alert('success',Lang::get('core.note_success'))
