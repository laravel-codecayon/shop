<?php
class AshippersController extends BaseController {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'ashippers';
	static $per_page	= '10';
	
	public function __construct() {
		parent::__construct();
		$this->model = new Ashippers();
		...
		$this->data = array(
			...
			'pageModule'		=> 'ashippers',
			'pageUrl'			=> URL::to('ashippers')		
		);
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','ashippers.index',$this->data);
	}		
	...
	{ 
		...
		return View::make('ashippers.table',$this->data);
	...
	function getAdd( $id = null)
	{
		...
		$this->data['id'] = $id;
		return View::make('ashippers.form',$this->data);
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		return View::make('ashippers.view',$this->data);
	...
	function postQuicksave( $id =0)
	{
		...
		$validator = Validator::make(Input::all(), $rules);	
		if ($validator->passes()) {	
			$data = $this->validatePost('ShipperID');
	...
	function postCopy()
	{
	    ...
	    foreach(DB::select("SHOW COLUMNS FROM shippers") as $column)
        {
			if( $column->Field != 'ShipperID')
				$columns[] = $column->Field;
        }
		$toCopy = implode(",",Input::get('id'));
		
				
		$sql = "INSERT INTO shippers (".implode(",", $columns).") ";
		$sql .= " SELECT ".implode(",", $columns)." FROM shippers WHERE ShipperID IN (".$toCopy.")";
		//DB::insert($sql);
		return Response::json(array(
			'status'=>'success',
			'message'=> SiteHelpers::alert('success',Lang::get('core.note_success'))
