<?php
class AemployeesController extends BaseController {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'aemployees';
	static $per_page	= '10';
	
	public function __construct() {
		parent::__construct();
		$this->model = new Aemployees();
		...
		$this->data = array(
			...
			'pageModule'		=> 'aemployees',
			'pageUrl'			=> URL::to('aemployees')		
		);
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','aemployees.index',$this->data);
	}		
	...
	{ 
		...
		return View::make('aemployees.table',$this->data);
	...
	function getAdd( $id = null)
	{
		...
		$this->data['id'] = $id;
		return View::make('aemployees.form',$this->data);
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		return View::make('aemployees.view',$this->data);
	...
	function postQuicksave( $id =0)
	{
		...
		$validator = Validator::make(Input::all(), $rules);	
		if ($validator->passes()) {	
			$data = $this->validatePost('EmployeeID');
	...
	function postCopy()
	{
		...
		$sql .= " SELECT ".implode(",", $columns)." FROM employees WHERE EmployeeID IN (".$toCopy.")";
		//DB::insert($sql);
		return Response::json(array(
			'status'=>'success',
			'message'=> SiteHelpers::alert('success',Lang::get('core.note_success'))
