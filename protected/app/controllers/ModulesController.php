...
class ModuleController extends BaseController {
	...
	public function __construct() {
		...
		$this->util = Session::get(md5('pcode'));
	...
	function postCreate()
	{
		...
		*/	
		return Redirect::to($this->module)
			->with('message', SiteHelpers::alert('success','created module disabled on demo site'))
			->withErrors($validator)->withInput();	
...
/*		$filename = app_path().'/moduleroutes.php';
  ...
  function postInstall( $id =0)
  {
	...
	public function postAuthen()
	{
		...
		$url = "http://www.sximobuilder.com/clients/authentication/get/?envantoname={$email}&envantocode={$code}";
		...
		if(!$result)
		{
			return Respone::json(array('status'=>'noconn',
								'message'=>' <h3> No Internet Connection </h3> The Builder require internet connection'));
		} else {
			if($validate['status'] == 'success')
			{
				$this->session->set_userdata('uname',$uname);
			//	$val = base64_encode(serialize($result));
				$this->session->set_userdata(md5('clientInfo:'.$uname),$validate);
				echo $result ;	
				//redirect('tools/builder.builder',301);	
			} else {
				echo $result ;
			}
		}  		
