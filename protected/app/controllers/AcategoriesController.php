<?php
class AcategoriesController extends BaseController {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'acategories';
	static $per_page	= '10';
	
	public function __construct() {
		parent::__construct();
		$this->model = new Acategories();
		...
		$this->data = array(
			...
			'pageModule'		=> 'acategories',
			'pageUrl'			=> URL::to('acategories')		
		);
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','acategories.index',$this->data);
	}		
	...
	{ 
		...
		return View::make('acategories.table',$this->data);
	...
	function getAdd( $id = null)
	{
		...
		$this->data['id'] = $id;
		return View::make('acategories.form',$this->data);
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		return View::make('acategories.view',$this->data);
	...
	function postQuicksave( $id =0)
	{
		...
		$validator = Validator::make(Input::all(), $rules);	
		if ($validator->passes()) {	
			$data = $this->validatePost('CategoryID');
	...
	function postCopy()
	{
	    ...
	    foreach(DB::select("SHOW COLUMNS FROM categories") as $column)
        {
		...
		$sql .= " SELECT ".implode(",", $columns)." FROM categories WHERE CategoryID IN (".$toCopy.")";
	//	DB::insert($sql);
		return Response::json(array(
			'status'=>'success',
			'message'=> SiteHelpers::alert('success',Lang::get('core.note_success'))
