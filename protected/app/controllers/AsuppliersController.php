<?php
class AsuppliersController extends BaseController {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'asuppliers';
	static $per_page	= '10';
	
	public function __construct() {
		parent::__construct();
		$this->model = new Asuppliers();
		...
		$this->data = array(
			...
			'pageModule'		=> 'asuppliers',
			'pageUrl'			=> URL::to('asuppliers')		
		);
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','asuppliers.index',$this->data);
	}		
	...
	{ 
		...
		return View::make('asuppliers.table',$this->data);
	...
	function getAdd( $id = null)
	{
		...
		$this->data['id'] = $id;
		return View::make('asuppliers.form',$this->data);
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		return View::make('asuppliers.view',$this->data);
	...
	function postQuicksave( $id =0)
	{
		...
		$validator = Validator::make(Input::all(), $rules);	
		if ($validator->passes()) {	
			$data = $this->validatePost('SupplierID');
	...
	function postCopy()
	{
	    ...
	    foreach(DB::select("SHOW COLUMNS FROM suppliers") as $column)
        {
		...
		$sql .= " SELECT ".implode(",", $columns)." FROM suppliers WHERE SupplierID IN (".$toCopy.")";
	//	DB::insert($sql);
		return Response::json(array(
			'status'=>'success',
			'message'=> SiteHelpers::alert('success',Lang::get('core.note_success'))
