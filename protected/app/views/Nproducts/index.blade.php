{{--*/ usort($tableGrid, "SiteHelpers::_sort") /*--}}
  <div class="page-content row">
    ...
    <div class="toolbar-line ">
			@if($access['is_add'] ==1)
	   		<a href="{{ URL::to('Nproducts/add?md='.$masterdetail["filtermd"].$trackUri) }}" class="tips btn btn-xs btn-info"  title="{{ Lang::get('core.btn_create') }}">
			...
			<a href="{{ URL::to('module/config/Nproducts') }}" class="tips btn btn-xs btn-default"  title="{{ Lang::get('core.btn_config') }}">
	...
	{{ $details }}
	 ...
	 {{ Form::open(array('url'=>'Nproducts/destroy/', 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) }}
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <td>
					...
					{{--*/ $id = SiteHelpers::encryptID($row->ProductID) /*--}}
				 	@if($access['is_detail'] ==1)
					<a href="{{ URL::to('Nproducts/show/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}"  class="tips btn btn-xs btn-primary"  title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search"></i> </a>
					@endif
					@if($access['is_edit'] ==1)
					<a  href="{{ URL::to('Nproducts/add/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}"  class="tips btn btn-xs btn-success"  title="{{ Lang::get('core.btn_edit') }}"> <i class="fa fa-edit"></i></a>
					@endif
