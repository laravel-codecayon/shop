{{--*/ usort($tableGrid, "SiteHelpers::_sort") /*--}}
  <div class="page-content row">
	...
	{{ $details }}
	 ...
	 {{ Form::open(array('url'=>'order/destroy/', 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) }}
    ...
    <table class="table table-striped ">
        ...
        <tbody>
            ...
            @foreach ($rowData as $row)
                <tr>
					<td width="50"> {{ ++$i }} </td>
					<td width="50"><input type="checkbox" class="ids" name="id[]" value="{{ $row->OrderID }}" />  </td>									
				 ...
				 <td>
					...
					{{--*/ $id = SiteHelpers::encryptID($row->OrderID) /*--}}
				 	@if($access['is_detail'] ==1)
					<a href="{{ URL::to('order/show/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}"  class="tips btn btn-xs btn-primary"  title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search"></i> </a>
					@endif
					@if($access['is_edit'] ==1)
					<a  href="{{ URL::to('order/add/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}"  class="tips btn btn-xs btn-success"  title="{{ Lang::get('core.btn_edit') }}"> <i class="fa fa-edit"></i></a>
					@endif
